...
export const QUIZZES: Record<number, QuizQuestion[]> = {
  ...
  10: [
    { q: 'An agent loop receives stop_reason "tool_use". What is the correct next step?', options: ['Return the text to the user', 'Execute the requested tool(s) and send tool_result blocks back in a user turn', 'Retry the request with a higher max_tokens', 'End the run'], answer: 1, explanation: 'tool_use means the model paused to call a tool. You execute it and return the result as a tool_result block in the next user message, then continue the loop.' },
    { q: 'Why should side-effecting tools carry an idempotency key?', options: ['To make schemas shorter', 'So a retried call after a timeout does not perform the action twice', 'To let the model skip the approval gate', 'To improve prompt caching'], answer: 1, explanation: 'Retries are inevitable. An idempotency key lets the backend recognise a duplicate and return the original result instead of sending or writing again.' },
    { q: 'Where should untrusted content such as an email body be placed?', options: ['In the system prompt', 'In a tool_result block, clearly delimited as data', 'Appended to the user instruction', 'Nowhere, agents cannot read email safely'], answer: 1, explanation: 'Untrusted content belongs in tool results, structured as data. The system prompt and user instructions are trusted channels and must never carry attacker-controlled text.' },
    { q: 'What does a trajectory eval assert on that an output eval does not?', options: ['Response length', 'The sequence of tool calls and their arguments', 'Token cost', 'Model version'], answer: 1, explanation: 'Trajectory evals grade the path: which tools ran, in what order, with what inputs. An agent can produce a correct final answer while taking a dangerous or wasteful path.' },
    { q: 'A tool your agent depends on is down. Which is the reliability-engineering response?', options: ['Crash the run and alert', 'Retry forever', 'Degrade gracefully: skip or substitute the capability, tell the user what was unavailable, and log it', 'Switch to a bigger model'], answer: 2, explanation: 'Partial failure is normal. The agent should produce the best result it can with what is available and be explicit about the gap.' },
  ],
}
